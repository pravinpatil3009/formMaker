...
if (theme_get_setting('fusemobile_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('fusemobile_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'fusemobile') .'/css/tabs.css', 'theme', 'screen');
}

// If we are displaying the hashgrid overlay add the js here.
if (theme_get_setting('fusemobile_hashgrid')) {
  drupal_add_css( drupal_get_path('theme', 'fusemobile') .'/css/hashgrid.css', 'theme', 'screen');
  drupal_add_js( drupal_get_path('theme', 'fusemobile') .'/scripts/hashgrid.js', 'theme');
}

// Add the css for wireframes around main layout elements
if (theme_get_setting('fusemobile_wireframe')) {
  drupal_add_css( drupal_get_path('theme', 'fusemobile') .'/css/wireframes.css', 'theme', 'screen');
}
...
function fusemobile_preprocess_page(&$vars, $hook) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = fusemobile_id_safe('page-'. $path);
    $body_classes[] = fusemobile_id_safe('section-'. $section);
...
function fusemobile_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = fusemobile_id_safe('node-type-' . $vars['type']);
...
function fusemobile_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = fusemobile_id_safe('block-' . $vars['block']->module);
    $classes[] = fusemobile_id_safe('block-' . $vars['block']->region);
    $classes[] = fusemobile_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('fusemobile_block_editing') && user_access('administer blocks')) {
...
function fusemobile_preprocess_comment(&$vars, $hook) {
...
function fusemobile_menu_item_link($link) {
...
function fusemobile_menu_local_tasks() {
  $output = '';
...
function fusemobile_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  ...
  $css_class = fusemobile_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function fusemobile_id_safe($string) {
...
function fusemobile_breadcrumb($breadcrumb) {
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('fusemobile_zen_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('fusemobile_zen_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('fusemobile_zen_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('fusemobile_zen_breadcrumb_title')) {
        if ($title = drupal_get_title()) {
          $trailing_separator = $breadcrumb_separator;
        }
      }
      elseif (theme_get_setting('fusemobile_zen_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
